...
def decode_chromosome(chromosome):
    ...
    for p in range(L_chromosome):
        ...
        value3+=(2**p)*chromosome[2][-1-p]


    #Conversión a un punto en el intervalo de busqueda
    return a+(b-a)*float(value1)/(N_chains-1), a+(b-a)*float(value2)/(N_chains-1), a+(b-a)*float(value3)/(N_chains-1)
...
def evaluate_chromosomes():
    ...
    for p in range(N_chromosomes):
        ...
        v1,v2,v3=decode_chromosome(F0[p])
        #Evaluamos el punto en la función y lo metemos en los valores de aptitud
        fitness_values[p] = rastrigin_function(v1,v2,v3)
        

def compare_chromosomes(chromosome1,chromosome2):
    vc1_str1, vc1_str2, vc1_str3=decode_chromosome(chromosome1)
    vc2_str1, vc2_str2, vc2_str3=decode_chromosome(chromosome2)
    #print(vc1_str1, vc1_str2)
    fvc1=rastrigin_function(vc1_str1, vc1_str2, vc1_str3)
    fvc2=rastrigin_function(vc2_str1, vc2_str2, vc2_str3)
    if fvc1 > fvc2:
        return 1
    elif fvc1 == fvc2:
        return 0
...
def nextgeneration():
    ...
    decode_value1, decode_value2, decode_value3 = decode_chromosome(F0[0])
    print(f'f({decode_value1}, {decode_value2}, {decode_value3}) = {rastrigin_function(decode_value1, decode_value2, decode_value3)}')
